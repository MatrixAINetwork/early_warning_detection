...
sensitivity <- function(timeseries, indicator = c("ar1", "sd", "acf1", "sk", 
  ...
  par(mar = (c(3, 3, 0.5, 2)),cex.axis = 1.5, cex.lab = 1.5,cex.main=1.5,oma = c(0.5, 1, 2, 1),mgp = c(2, 0.6, 0),
      tcl = 0.5,font.main = 10)
 # par(font.main = 10, mar = (c(4.6, 4, 0.5, 2) + 0.2), mgp = c(2, 1, 0), oma = c(0.5, 1, 2, 1))
  plot(tw, Ktauestind, type = "l", ylim = c(-1, 1), log = "x", xlab = "rolling window size", 
       ylab = "Kendall tau estimate",lwd=3)
  ...
  #      ylab = "occurence", border = "black", xlim = c(0, max(Ktaupind)))
  indic_name =toupper(indicator)
  if(indicator == 'acf1'){
    indic_name = 'Autocorrelation(1)'
  }
  if(indicator == 'sd'){
    indic_name = 'Variance'
  }
  ...
  mtext(paste("Indicator ", indic_name), side = 3, line = 0.2, outer = TRUE,cex = 1.5)
